...
namespace Peacock.ZXEval.Repository
{
    public abstract class Repository<TEntity, TSing> where TEntity : class
    {
        ...
        private static readonly string _readKey = "_ZXDbContextKey";
        private static Collection CoreContext
        {
            get
            {
                ...
                if (collection == null)
                {
                    collection = new Collection(string.Format("{0}", ConfigurationManager.ConnectionStrings["ZXReadConnection"]));
        ...
        /// 执行查询类SQL语句
        /// </summary>
        /// <tyZXaram name="TEntity"></tyZXaram>
        ...
        /// <param name="paramers"></param>
        /// <returns></returns>
        public virtual int ExecuteSqlCommand(string sql, params object[] paramers)
        {
            return CoreContext.Database.ExecuteSqlCommand(sql, paramers);
        }
        ...
        public virtual IQueryable<TEntity> FindAsNoTracking(Expression<Func<TEntity, bool>> expression)
        {
            return CoreContext.Set<TEntity>().AsNoTracking().Where(expression);
        }
    }
}