...
//全局变量
var str = '<div class="tab-layout" id="tab-layout"><div class="tab-layout-title"><div class="tab-layout-left-arrow" onclick="prev()"><i class="fa fa-caret-left"></i></div> <div class="tab-layout-right-arrow" onclick="next()"><i class="fa fa-caret-right"></i></div><div class="tab-layout-content"><ul></ul></div> </div><div class="tab-layout-body"></div></div>';
...
function additem(id, name, url, _del) {
    ...
    if (isHas) {
        ...
        if (__width <= _headerWidth) {
            ...
            tll_arrow.removeAttr("disabled");
            tlr_arrow.removeAttr("disabled");
        }
        tabContent.animate({
        ...
        if (_del == "true") {
            ...
            str_title = '<li class="item-content current" onclick=select("' + _id + '",event) id="' + _id + '"><a href="javascript:void(0);">' + name + '</a><span class="item-del fa fa-close" onClick=delitem("' + _id + '",event)>&nbsp;</span></li>';
        }
...
function showFrame(isHas, id, url) {
    ...
    if (isHas) {
        ...
        if (currentUrl != url) { //刷新
            ...
            var currentIframe = $("#" + iFrameID)[0].contentWindow;
            if (currentIframe && typeof (currentIframe.search) == typeof (Function)) {
                currentIframe.search();
            }
        }

    } else { //新增
        _height = $(window).height() - $(container).find(".tab-layout-title").height() - $(container).offset().top + 10;
        _height = (_height <= 0) ? 0 : _height;
        ifBod.append('<iframe id="' + iFrameID + '" name="' + iFrameID + '" class="iFrameShow" scrolling="yes" frameborder="0" src="' + url + '" style="height:' + _height + 'px"></iframe>');
        //加入缓存
        var iFrame = {
            id: iFrameID,
            url: url
...
function delitem(id, e) {
    var _id = id;
    var preid;
    var objprev = (tabContent.find("li#" + id).prev().length == 0) ? null : tabContent.find("li#" + id).prev();
    var obknext = (tabContent.find("li#" + id).next().length ==0) ? null : tabContent.find("li#" + id).next();
    var __id = getSelectedID();
    selectedWidth = $('.tab-layout-content ul li#' + _id, header).outerWidth(true);
...
function resizeWin() {
    _headerWidth = header.width();
    _height = $(window).height() - $(container).find(".tab-layout-content").height() - $(container).offset().top + 10;
    _height = (_height <= 0) ? 0 : _height;
