...
namespace Peacock.ZXEval.Data.Entities
{
    ...
    public class Project
    {
        ...
        //外采数据
        public virtual OuterTask OuterTask { get; set; }

        /// <summary>
        /// 与状态的对应关系
        /// </summary>
        public virtual ICollection<ProjectStateInfo> ProjectStateInfos { get; set; }
    ...
    internal class ProjectConfig : EntityConfig<Project>
    {
        internal ProjectConfig()
        {
            ToTable("Project");
            HasKey(p => p.Id);
            //HasOptional(x => x.Customer).WithMany().HasForeignKey(c => c.CustomerId);
            ...
            HasRequired(x => x.OuterTask).WithRequiredPrincipal(x => x.Project).Map(x => x.MapKey("ProjectID"));
            HasMany(x => x.ProjectStateInfos).WithRequired(x => x.Project).HasForeignKey(x => x.ProjectId);
            HasMany(x => x.ProjectResources).WithRequired().HasForeignKey(x => x.ProjectId);
        }
    }
}
